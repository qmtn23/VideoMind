...
import ChunkListDrawer from './modules/chunk-list-drawer.vue';
import SummaryModal from './modules/summary-modal.vue';
...
function renderIcon(fileName: string) {
  ...
  if (ext) {
    if (VIDEO_EXTS.has(ext)) return <SvgIcon localIcon="video" class="mx-4 text-12" />;
      ...
      render: row => (
        <div class="flex gap-4">
          ...
          <NButton
            ...
            ghost
            size="small"
            onClick={() => { chunkTargetMd5.value = row.fileMd5; chunkTargetName.value = row.fileName; chunkDrawerVisible.value = true; }}
          >
            分块
          </NButton>
          {isVideoFile(row.fileName) && (
            <NButton
              type="warning"
              ghost
              size="small"
              onClick={() => { summaryTargetMd5.value = row.fileMd5; summaryTargetName.value = row.fileName; summaryVisible.value = true; }}
            >
              摘要
            </NButton>
          )}
...
const chunkTargetMd5 = ref('');
const chunkTargetName = ref('');
// #endregion

// #region 视频摘要
const summaryVisible = ref(false);
const summaryTargetMd5 = ref('');
...
<template>
  <div class="min-h-500px flex-col-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    ...
    <ChunkListDrawer v-model:visible="chunkDrawerVisible" :file-md5="chunkTargetMd5" :file-name="chunkTargetName" />
    <SummaryModal v-model:visible="summaryVisible" :file-md5="summaryTargetMd5" :file-name="summaryTargetName" />
