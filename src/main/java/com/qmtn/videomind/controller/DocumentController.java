...
import com.qmtn.videomind.model.OrganizationTag;
import com.qmtn.videomind.repository.DocumentVectorRepository;
import com.qmtn.videomind.repository.FileUploadRepository;
import com.qmtn.videomind.repository.OrganizationTagRepository;
import com.qmtn.videomind.service.DocumentService;
import com.qmtn.videomind.utils.LogUtils;
import com.qmtn.videomind.utils.JwtUtils;
...
public class DocumentController {
    ...
    /**
     * 获取指定文件的所有文本分块（不含向量数据）
     *
     * @param fileMd5 文件MD5
     * @return 分块列表，每项包含 chunkId 和 textContent
     */
    @GetMapping("/{fileMd5}/chunks")
    public ResponseEntity<?> getChunks(@PathVariable String fileMd5) {
        try {
            List<DocumentVector> chunks = documentVectorRepository.findByFileMd5OrderByChunkIdAsc(fileMd5);
            List<Map<String, Object>> result = chunks.stream().map(c -> Map.<String, Object>of(
            ...
            Map<String, Object> response = new HashMap<>();
            response.put("code", HttpStatus.INTERNAL_SERVER_ERROR.value());
            response.put("message", "获取分块失败: " + e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
        }
    }
