 ...
 * 视频格式始终纳入支持列表；上传时若 DashScope ASR 未就绪则拒绝视频。
 */
@Service
public class FileTypeValidationService {
    ...
    @PostConstruct
    public void init() {
        SUPPORTED_DOCUMENT_EXTENSIONS.addAll(VIDEO_EXTENSIONS);
        UNSUPPORTED_EXTENSIONS.removeAll(VIDEO_EXTENSIONS);
        logger.info("视频格式已加入支持列表（解析依赖 DashScope ASR）: {}", VIDEO_EXTENSIONS);
    }
    ...
    public FileTypeValidationResult validateFileType(String fileName) {
        ...
        if (VIDEO_EXTENSIONS.contains(extension) && !dashScopeAsrClient.isEnabled()) {
            String message = "视频解析为平台核心能力，需要 asr.enabled=true（默认开启）且配置有效的 asr.api.key，"
                    + "并安装 FFmpeg、配置公网可访问的 minio.publicUrl（DashScope 需拉取临时音频）。";
            logger.warn("文件类型验证失败: fileName={}, extension={}, reason=asr_not_ready", fileName, extension);
