...
<cite>
**本文档中引用的文件**   
- [AuthController.java](file://src/main/java/com/qmtn/videomind/controller/AuthController.java)
- [ChatController.java](file://src/main/java/com/qmtn/videomind/controller/ChatController.java)
- [DocumentController.java](file://src/main/java/com/qmtn/videomind/controller/DocumentController.java)
- [SearchController.java](file://src/main/java/com/qmtn/videomind/controller/SearchController.java)
- [WebSocketConfig.java](file://src/main/java/com/qmtn/videomind/config/WebSocketConfig.java)
- [ChatWebSocketHandler.java](file://src/main/java/com/qmtn/videomind/handler/ChatWebSocketHandler.java)
- [ChatHandler.java](file://src/main/java/com/qmtn/videomind/service/ChatHandler.java)
- [DocumentService.java](file://src/main/java/com/qmtn/videomind/service/DocumentService.java)
- [HybridSearchService.java](file://src/main/java/com/qmtn/videomind/service/HybridSearchService.java)
- [JwtUtils.java](file://src/main/java/com/qmtn/videomind/utils/JwtUtils.java)
- [SecurityConfig.java](file://src/main/java/com/qmtn/videomind/config/SecurityConfig.java)
- [JwtAuthenticationFilter.java](file://src/main/java/com/qmtn/videomind/config/JwtAuthenticationFilter.java)
- [CustomUserDetailsService.java](file://src/main/java/com/qmtn/videomind/service/CustomUserDetailsService.java)
- [User.java](file://src/main/java/com/qmtn/videomind/model/User.java)
- [FileUpload.java](file://src/main/java/com/qmtn/videomind/model/FileUpload.java)
- [SearchResult.java](file://src/main/java/com/qmtn/videomind/entity/SearchResult.java)
- [EsDocument.java](file://src/main/java/com/qmtn/videomind/entity/EsDocument.java)
- [Conversation.java](file://src/main/java/com/qmtn/videomind/model/Conversation.java)
</cite>
...
## 简介
本文档为VideoMind智能问答系统提供完整的RESTful API参考，涵盖用户认证、文档管理、聊天交互和知识库搜索等所有公开接口。系统采用Spring Boot后端框架，结合Vue 3前端，实现了一个基于知识库的智能问答平台。后端通过JWT进行用户认证，利用Elasticsearch实现混合搜索（文本匹配+向量相似度），并通过WebSocket提供实时聊天功能。API设计遵循RESTful原则，所有响应均采用统一的JSON格式，包含`code`、`message`和`data`字段。文档详细记录了每个端点的HTTP方法、URL路径、请求参数、请求头和响应结构，并提供了curl示例和Postman调用场景。

## 项目结构
VideoMind项目采用前后端分离的架构。后端位于`src/main/java`目录，基于Spring Boot构建，主要包含`controller`（控制器）、`service`（服务）、`repository`（数据访问）和`model`（实体）等包。前端位于`frontend/src`目录，使用Vue 3和TypeScript开发，包含`components`（组件）、`views`（视图）、`store`（状态管理）和`service`（API调用）等模块。配置文件如`application.yml`定义了数据库、Redis和MinIO等外部服务的连接信息。`pom.xml`文件管理了所有Java依赖。
...
**节源**
- [JwtUtils.java](file://src/main/java/com/qmtn/videomind/utils/JwtUtils.java#L1-L50)
- [ChatHandler.java](file://src/main/java/com/qmtn/videomind/service/ChatHandler.java#L1-L50)
- [HybridSearchService.java](file://src/main/java/com/qmtn/videomind/service/HybridSearchService.java#L1-L50)
...
**图源**
- [SecurityConfig.java](file://src/main/java/com/qmtn/videomind/config/SecurityConfig.java#L1-L20)
- [ChatController.java](file://src/main/java/com/qmtn/videomind/controller/ChatController.java#L1-L20)
...
**图源**
- [JwtAuthenticationFilter.java](file://src/main/java/com/qmtn/videomind/config/JwtAuthenticationFilter.java#L1-L50)
- [JwtUtils.java](file://src/main/java/com/qmtn/videomind/utils/JwtUtils.java#L1-L50)
...
- **curl示例**:
  ```bash
  curl -X POST https://api.videomind.com/api/v1/auth/refreshToken \
    -H "Content-Type: application/json" \
    -d '{"refreshToken": "your_refresh_token"}'
  ```

**节源**
- [AuthController.java](file://src/main/java/com/qmtn/videomind/controller/AuthController.java#L1-L85)
...
**图源**
- [DocumentController.java](file://src/main/java/com/qmtn/videomind/controller/DocumentController.java#L1-L50)
- [DocumentService.java](file://src/main/java/com/qmtn/videomind/service/DocumentService.java#L1-L50)
...
- **curl示例**:
  ```bash
  curl -X DELETE https://api.videomind.com/api/v1/documents/abc123... \
...
**节源**
- [DocumentController.java](file://src/main/java/com/qmtn/videomind/controller/DocumentController.java#L50-L200)
...
#### WebSocket接口
**连接URL**: `wss://api.videomind.com/chat/{jwt_token}`
- **消息格式**: 发送纯文本消息。
...
**节源**
- [ChatController.java](file://src/main/java/com/qmtn/videomind/controller/ChatController.java#L1-L81)
- [ChatWebSocketHandler.java](file://src/main/java/com/qmtn/videomind/handler/ChatWebSocketHandler.java#L1-L121)
...
**图源**
- [SearchController.java](file://src/main/java/com/qmtn/videomind/controller/SearchController.java#L1-L90)
- [HybridSearchService.java](file://src/main/java/com/qmtn/videomind/service/HybridSearchService.java#L1-L50)
...
- **curl示例**:
  ```bash
  curl -G https://api.videomind.com/api/search/hybrid \
    -H "Authorization: Bearer your_jwt_token" \
...
```mermaid
graph TD
A[VideoMindApplication] --> B[Spring Boot]
A --> C[Spring Security]
A --> D[Spring Data JPA]
A --> E[Spring Web]
A --> F[Spring WebFlux]
...
**节源**
- [JwtAuthenticationFilter.java](file://src/main/java/com/qmtn/videomind/config/JwtAuthenticationFilter.java#L1-L98)
- [ChatHandler.java](file://src/main/java/com/qmtn/videomind/service/ChatHandler.java#L1-L199)

## 结论
本文档详细介绍了VideoMind系统的RESTful API，涵盖了从用户认证到知识库搜索的完整流程。系统架构清晰，组件职责分明，通过JWT实现安全认证，利用Elasticsearch和大模型提供智能问答能力。API设计规范，易于第三方集成。开发者在使用时，应重点关注认证流程、WebSocket消息格式和搜索接口的权限控制。未来可考虑增加API版本控制和更详细的错误码定义，以进一步提升API的健壮性和易用性。